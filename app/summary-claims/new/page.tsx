    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4ff" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "48px 40px", textAlign: "center", boxShadow: "0 4px 24px rgba(0,82,204,0.10)", maxWidth: 380, width: "100%", border: "1px solid #dce7ff" }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0a2540", marginBottom: 8 }}>Summary Saved</h2>
          <p style={{ color: "#64748b", fontSize: 14, marginBottom: 28 }}>The summary of claims has been recorded successfully.</p>
          <button onClick={() => router.push(`/summary-claims/${savedId}`)} style={{ width: "100%", background: "#0052cc", color: "#fff", border: "none", borderRadius: 8, padding: "11px 0", fontWeight: 700, fontSize: 14, cursor: "pointer", marginBottom: 10 }}>
            View &amp; Print Summary
          </button>
          <button onClick={() => setSavedId(null)} style={{ width: "100%", background: "transparent", color: "#0052cc", border: "1.5px solid #0052cc", borderRadius: 8, padding: "10px 0", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff", fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "32px 24px 60px" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0a2540", margin: 0 }}>New Summary of Claims</h1>
          <p style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>Fill in the claim details and staff entries below, then save to generate the printable summary.</p>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
            <div>
              ...
              <input value={district} onChange={(e) => setDistrict(e.target.value)} style={inputStyle} placeholder="e.g. Obuasi" />
            </div>
            <div>
              <label style={labelStyle}>Month</label>
              <input value={month} onChange={(e) => setMonth(e.target.value)} style={inputStyle} placeholder="e.g. June 2026" />
            </div>
            <div>
              <label style={labelStyle}>Prepared By</label>
              <input value={preparedBy} onChange={(e) => setPreparedBy(e.target.value)} style={inputStyle} placeholder="Name of preparer" />
            </div>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, paddingBottom: 10, borderBottom: "2px solid #e8f0ff" }}>
            <div style={sectionTitleStyle}>② Staff Entries</div>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#64748b" }}>
              <div style={{ width: 14, height: 14, background: "#fff", border: "1px solid #dce7ff", borderRadius: 3 }} /> Proposed value
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#64748b" }}>
              <div style={{ width: 14, height: 14, background: "#f0f4ff", border: "1px solid #dce7ff", borderRadius: 3 }} /> Approved by RCM
            </div>
          </div>
                  ...
                  <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#f8faff" }}>
                    ...
                    <td style={{ ...tdStyle, textAlign: "center", minWidth: 80 }}>
                      ...
                      {row.overtimeCategory && (
                        <div style={{ marginTop: 3, fontSize: 9, fontWeight: 700, color: "#fff", background: "#0052cc", borderRadius: 4, padding: "1px 6px", display: "inline-block" }}>
                    ...
                    <td style={{ ...tdStyle, textAlign: "center" }}>
                      {staffRows.length > 1 && (
                        <button onClick={() => removeRow(i)} title="Remove row" style={{ background: "#fee2e2", border: "none", borderRadius: 4, color: "#dc2626", fontWeight: 700, fontSize: 14, width: 24, height: 24, cursor: "pointer", lineHeight: 1 }}>
                          ×
                        </button>
                      )}
                    </td>
          ...
          <button
            type="button"
            onClick={addRow}
            style={{ marginTop: 14, border: "1.5px dashed #0052cc", color: "#0052cc", background: "transparent", fontSize: 12, fontWeight: 600, padding: "7px 18px", borderRadius: 6, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}
          >
            + Add Staff Row
          </button>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 8 }}>
          ...
          <button
            type="button"
            onClick={handleSave}
            style={{ background: "#0052cc", color: "#fff", border: "none", borderRadius: 8, padding: "12px 32px", fontWeight: 700, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, boxShadow: "0 4px 14px rgba(0,82,204,0.35)" }}
          >
