      ...
      <div style={{ background: "linear-gradient(135deg, #003d99 0%, #0052cc 55%, #1a6bff 100%)", padding: "36px 32px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <p style={{ color: "#c0d6ff", fontSize: 13, margin: 0 }}>
            View, filter, and print all travel vouchers and summary of claims.
          </p>
        </div>
      </div>
        ...
        <div style={sectionCard}>
          <div style={{ background: "linear-gradient(90deg, #003d99, #0052cc)", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              />
              {filterMonth && (
                <button onClick={() => setFilterMonth("")} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", borderRadius: 6, padding: "6px 10px", fontSize: 12, cursor: "pointer" }}>✕ Clear</button>
              )}
            </div>
          </div>
          ...
          {filteredVouchers.length === 0 ? (
            <div style={{ padding: "40px 24px", textAlign: "center" }}>
              ...
              <div style={{ color: "#64748b", fontSize: 14 }}>
                {filterMonth ? `No vouchers found for "${filterMonth}".` : "No vouchers yet."}
              </div>
            </div>
          ) : (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#f0f4ff", borderBottom: "2px solid #dce7ff" }}>
                    {["#", "Employee", "District", "Month", "Date", "Total (GH¢)", "Action"].map((h) => (
          ...
          {summaries.length === 0 ? (
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#f0f4ff", borderBottom: "2px solid #dce7ff" }}>
                    {["#", "District", "Month", "Prepared By", "Date", "Action"].map((h) => (
                      <th key={h} style={tableHeader}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {summaries.map((s, idx) => (
                    <tr key={s.id} style={{ borderBottom: "1px solid #f0f4ff", background: idx % 2 === 0 ? "#fff" : "#f8faff" }}>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <Link href={`/summary-claims/${s.id}`} style={{ background: "#1a6bff", color: "#fff", padding: "5px 14px", borderRadius: 6, fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
                          View & Print
                        </Link>
                      </td>
                    </tr>
