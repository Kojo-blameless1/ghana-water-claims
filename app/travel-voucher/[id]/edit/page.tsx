...
export default function EditTravelVoucher() {
  const { id } = useParams();
  const { register, handleSubmit, watch, getValues, reset, formState: { errors } } = useForm<FormValues>();
    ...
    fetch(`/api/travel-voucher/${id}`)
      ...
      .then((data) => {
        reset({
          ...
          hotelPerNight: data.hotelPerNight,
          byAir: data.byAir,
          byRail: data.byRail,
          privateVehicleMiles: data.privateVehicleMiles,
        ...
        const entries = JSON.parse(data.itineraryEntries || "[]");
        setRows(entries.length > 0 ? entries : [emptyRow()]);
        setLoading(false);
      });
  }, [id]);
  ...
  const removeRow = (idx: number) => { if (rows.length > 1) setRows(rows.filter((_, i) => i !== idx)); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/travel-voucher/${id}`, {
        method: "PUT",
  ...
  if (loading)
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4ff", fontFamily: "'Inter','Segoe UI',sans-serif" }}>
        <div style={{ color: "#0052cc", fontWeight: 600 }}>Loading voucher…</div>
      </div>
    );
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4ff", fontFamily: "'Inter','Segoe UI',sans-serif" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "48px 40px", textAlign: "center", maxWidth: 380, width: "100%", border: "1px solid #dce7ff", boxShadow: "0 4px 24px rgba(0,82,204,0.10)" }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0a2540", marginBottom: 8 }}>Voucher Updated</h2>
          <p style={{ color: "#64748b", fontSize: 14, marginBottom: 28 }}>Changes saved successfully.</p>
          <button onClick={() => router.push(`/travel-voucher/${id}`)} style={{ width: "100%", background: "#0052cc", color: "#fff", border: "none", borderRadius: 8, padding: "11px 0", fontWeight: 700, fontSize: 14, cursor: "pointer", marginBottom: 10 }}>
            View & Print
          </button>
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff", fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{ background: "#0052cc", padding: "0 32px", display: "flex", alignItems: "center", gap: 16, height: 60, boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ background: "rgba(255,255,255,0.15)", color: "#fff", fontSize: 12, fontWeight: 600, padding: "4px 14px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.3)" }}>
            Edit Voucher #{id}
          </span>
          <button onClick={() => router.push("/admin")} style={{ background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 6, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
            ← Dashboard
          </button>
        </div>
      </div>
        ...
        {[{ n: 1, label: "Voucher Details" }, { n: 2, label: "Itinerary" }].map((s, i) => (
          <div key={s.n} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, background: step === s.n ? "#0052cc" : step > s.n ? "#0052cc" : "#e8f0ff", color: step >= s.n ? "#fff" : "#0052cc" }}>
                {step > s.n ? "✓" : s.n}
              </div>
              <span style={{ fontSize: 13, fontWeight: step === s.n ? 700 : 500, color: step === s.n ? "#0052cc" : "#94a3b8" }}>{s.label}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "32px 24px 60px" }}>
        ...
        {step === 1 && (
          <>
            <div style={{ marginBottom: 24 }}>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0a2540", margin: 0 }}>Edit Travel Expense Voucher</h1>
              <p style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>Update the voucher details then proceed to the itinerary.</p>
            </div>

            <div style={sectionStyle}>
              <div style={sectionTitleStyle}>① Employee Details</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div><label style={labelStyle}>Employee Name</label><input {...register("employee", { required: true })} style={inputStyle} /></div>
                <div><label style={labelStyle}>Post / Title</label><input {...register("post")} style={inputStyle} /></div>
                <div><label style={labelStyle}>District</label><input {...register("district")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Activity</label><input {...register("activity")} style={inputStyle} /></div>
                <div style={{ gridColumn: "1 / -1" }}><label style={labelStyle}>Purpose of Travel</label><input {...register("purpose")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Allowance for the Month</label><input {...register("allowanceMonth")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Date</label><input type="date" {...register("date")} style={inputStyle} /></div>
              </div>
            </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, alignItems: "end" }}>
                <div><label style={labelStyle}>Number of Nights</label><input type="number" min={0} {...register("hotelNights")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Rate per Night (GH¢)</label><input type="number" step="0.01" min={0} {...register("hotelPerNight")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Lodging Actual (auto)</label><div style={readonlyStyle}>GH¢ {hotelActual.toFixed(2)}</div></div>
              </div>
            </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div><label style={labelStyle}>By Air (GH¢)</label><input type="number" step="0.01" min={0} {...register("byAir")} style={inputStyle} /></div>
                <div><label style={labelStyle}>By Rail (GH¢)</label><input type="number" step="0.01" min={0} {...register("byRail")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Private Vehicle — Miles</label><input type="number" min={0} {...register("privateVehicleMiles")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Rate per Mile (GH¢)</label><input type="number" step="0.01" min={0} {...register("privateVehicleRate")} style={inputStyle} /></div>
                {vehicleTotal > 0 && (
                ...
                <div><label style={labelStyle}>Tolls etc. (GH¢)</label><input type="number" step="0.01" min={0} {...register("tolls")} style={inputStyle} /></div>
                <div><label style={labelStyle}>Miscellaneous (GH¢)</label><input type="number" step="0.01" min={0} {...register("miscellaneous")} style={inputStyle} /></div>
              </div>
            </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "end" }}>
                <div><label style={labelStyle}>Account Code</label><input {...register("accountCode")} style={inputStyle} /></div>
                <div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button type="button" onClick={goToStep2} style={{ background: "#0052cc", color: "#fff", border: "none", borderRadius: 8, padding: "12px 32px", fontWeight: 700, fontSize: 14, cursor: "pointer", boxShadow: "0 4px 14px rgba(0,82,204,0.35)" }}>
                Next: Itinerary →
              </button>
            </div>
          </>
        ...
        {step === 2 && (
          <>
            <div style={{ marginBottom: 24 }}>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0a2540", margin: 0 }}>Edit Itinerary</h1>
              <p style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>Update each leg of the journey.</p>
            </div>
            ...
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #dce7ff", boxShadow: "0 1px 8px rgba(0,82,204,0.06)", padding: "24px 28px", marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, paddingBottom: 10, borderBottom: "2px solid #e8f0ff" }}>
                ...
                <span style={{ background: "#e8f0ff", color: "#0052cc", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20 }}>{rows.length} row{rows.length !== 1 ? "s" : ""}</span>
              </div>
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 980 }}>
                  <thead>
                    ...
                    <tr>
                      <th style={thSub}></th>
                      <th style={thSub}>Place</th><th style={thSub}>Date</th><th style={thSub}>Hour</th>
                      <th style={thSub}>Place</th><th style={thSub}>Date</th><th style={thSub}>Hour</th>
                      <th style={thSub}>Standard</th><th style={thSub}>Sub-standard</th>
                      <th style={thSub}></th><th style={thSub}></th><th style={thSub}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((row, idx) => (
                      <tr key={idx} style={{ background: idx % 2 === 0 ? "#fff" : "#f8faff" }}>
                        ...
                        <td style={{ ...td, background: "#f8faff" }}><input value={row.depPlace} onChange={e => updateRow(idx, "depPlace", e.target.value)} style={cellInput} /></td>
                        ...
                        <td style={{ ...td, background: "#f0f4ff" }}><input value={row.arrPlace} onChange={e => updateRow(idx, "arrPlace", e.target.value)} style={{ ...cellInput, background: "transparent" }} /></td>
                        ...
                        <td style={td}><input type="number" value={row.mileageStandard || ""} onChange={e => updateRow(idx, "mileageStandard", parseFloat(e.target.value) || 0)} style={{ ...cellInput, textAlign: "right" }} /></td>
                        <td style={td}><input type="number" value={row.mileageSubstandard || ""} onChange={e => updateRow(idx, "mileageSubstandard", parseFloat(e.target.value) || 0)} style={{ ...cellInput, textAlign: "right" }} /></td>
                        <td style={td}><input type="number" value={row.radius || ""} onChange={e => updateRow(idx, "radius", parseFloat(e.target.value) || 0)} style={{ ...cellInput, textAlign: "right" }} /></td>
                        <td style={td}><input type="number" step="0.01" value={row.conveyanceAmount || ""} onChange={e => updateRow(idx, "conveyanceAmount", parseFloat(e.target.value) || 0)} style={{ ...cellInput, textAlign: "right" }} /></td>
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={7} style={{ border: "1px solid #dce7ff", padding: "8px 12px", textAlign: "right", fontWeight: 700, fontSize: 12, color: "#0052cc", background: "#e8f0ff" }}>TOTAL MILEAGE</td>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 8 }}>
              <button type="button" onClick={() => setStep(1)} style={{ background: "transparent", color: "#0052cc", border: "1.5px solid #0052cc", borderRadius: 8, padding: "11px 24px", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
                ← Back to Voucher
              </button>
              <button type="button" onClick={handleSave} disabled={saving} style={{ background: saving ? "#7aa7e0" : "#0052cc", color: "#fff", border: "none", borderRadius: 8, padding: "12px 32px", fontWeight: 700, fontSize: 14, cursor: saving ? "not-allowed" : "pointer", boxShadow: "0 4px 14px rgba(0,82,204,0.35)" }}>
